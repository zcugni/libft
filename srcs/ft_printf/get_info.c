...
void	get_info(t_pos *p, t_detail *conv_det, va_list ap)
{
	...
	while (str[++(p->i)] && (str[p->i] == 35 || str[p->i] == 48 ||
		...
		ft_lstadd(&(conv_det->info), m_lstnew(&str[p->i], 1, 1));
...
int		get_nb(const char *str, int *i)
{
	...
	while (str[*i] && str[*i] >= 48 && str[*i] <= 57)
		(*i)++;
	nb_string = m_strsub((char *)str, start, *i - start, 0);
...
void	get_width_modifier(const char **str, t_list **info, int *i)
{
		...
		(*str)[*i] == 'j' || (*str)[*i] == 'z')
	{
		...
		{
			tmp = m_strndup(&(*str)[(*i)++], 2);
			ft_lstadd(info, m_lstnew(tmp, 3, 1));
		}
		else
		{
			tmp = m_strndup(&(*str)[(*i)], 1);
			ft_lstadd(info, m_lstnew(tmp, 2, 1));
		}
		(*i)++;
		free(tmp);
	}
	else
		ft_lstadd(info, m_lstnew(NULL, 0, 1));
}
