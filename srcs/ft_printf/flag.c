...
static void	define_holder(t_detail conv_detail, char *chara)
{
	if ((conv_detail.pre == 0 ||
		...
		&& m_lstfindi(conv_detail.info, "0", 1) > -1
		&& m_lstfindi(conv_detail.info, "-", 1) != -1)
		*chara = 48;
	else
		*chara = 32;
}
...
static void	add_signed(char type, t_detail *conv_det)
{
	...
	res = conv_det->conv;
	if (m_lstfindi(conv_det->info, "+", 1) > -1
			&& (type == 'i' || type == 'd' || type == 'D')
			&& !is_neg(res->str[0]))
		res->str[0] = m_strjoin("+", res->str[0], 1);
	if (m_lstfindi(conv_det->info, " ", 1) > -1
			&& m_lstfindi(conv_det->info, "+", 1) != -1
			&& (type == 'i' || type == 'd' || type == 'D')
			&& !is_neg(res->str[0]))
...
static int	init(char *type, t_detail *conv_de, char **to_add, char *chara)
{
	...
	*type = ((char *)(conv_de->info->content))[0];
	if (conv_de->pre < 0 && conv_de->conv->str[0][0] != 48
			&& m_lstfindi(conv_de->info, "#", 1) > -1 && *type != 's')
		conv_de->pre++;
	def_prefix(conv_de, to_add);
	pad_precision(*conv_de, *type);
	define_holder(*conv_de, chara);
	if (m_lstfindi(conv_de->info, "+", 1) > -1
	|| m_lstfindi(conv_de->info, " ", 1) > -1)
		if ((*type == 'i' || *type == 'd' || *type == 'D')
		&& ft_atoi(res->str[0]) >= 0 && *chara == '0')
			conv_de->width--;
	if (m_lstfindi(conv_de->info, "#", 1) > -1
	&& m_lstfindi(conv_de->info, "0", 1) > -1
			&& m_lstfindi(conv_de->info, "-", 1) == -1 && (*type == 'x'
...
void		act_on_flag(t_detail *conv_det)
{
	...
	if (conv_det->conv->str[0])
	{
		...
		if (chara == 32 && to_add != NULL)
			conv_det->conv->str[0] =
				m_strjoin(to_add, conv_det->conv->str[0], 1);
		if (chara == 48)
		{
			if (type == 'p')
				conv_det->width -= 2;
